...
//Linea de arriba se modifica cuando después creo el ROOM
        ...
        init{
            ...
            val proDao = DataBaseProductos.getDataBase(application).getproductosDao()



            mRepository= Repository(proDao)
            //indico el método que traerá el repository
            mRepository.getDataFromServer(mprod=0)
            mRepository.getDataFromServerdeta(mdeta=0)
        }
        //primer fragmento
...
//segundo fragmanto
        ...
        fun getOnedetallekByID(id:Int): LiveData<List<DetalleEntity>> {
            return mRepository.getOnedetallekByID(id)
        }
